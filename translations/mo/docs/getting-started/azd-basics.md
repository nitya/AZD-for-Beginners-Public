...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-24T16:42:59+00:00",
...
本地 `.azure` 文件夾
緩存的部署信息
防止 azd "記住" 之前的部署，避免資源群組不匹配或過期的註冊表引用等問題。

### 為什麼要同時使用？
當您因殘留狀態或部分部署而在使用 `azd up` 時遇到障礙時，這個組合可以確保獲得 **乾淨的起點**。
...
理解身份驗證對於成功的 azd 部署至關重要。Azure 使用多種身份驗證方法，而 azd 利用與其他 Azure 工具相同的憑證鏈。
...
**📖 繼續第 1 章學習：**
- [安裝與設定](installation.md) - 安裝和配置 azd
